...
sw.addEventListener("sync", (event: any) => {
  if (event.tag === "sync-pedidos") {
    ...
    event.waitUntil((sw as any).syncPedidos?.() ?? Promise.resolve());
  }
});
...
self.addEventListener("message", (event: any) => {
  if (event.data === "SYNC_PEDIDOS_NOW") {
    event.waitUntil?.((self as any).syncPedidos?.() ?? Promise.resolve());
  }
});
...
async function cacheFirstUpdateImages(request: Request) {
